...
var loadConfigFile = function (type) {
	var json = null, managerPath = ".";
	type = (typeof type === "undefined") ? "user" : type;
	
        if (window.filemanager.options.managerPath)
            managerPath=window.filemanager.options.managerPath;

	if(type == 'user') {
		var url = managerPath + '/scripts/filemanager.config.js';
	} else {
		var url = managerPath + '/scripts/filemanager.config.js.default';
	}
...
// we merge default config and user config file
    var config = $.extend(true, {}, config, window.filemanager);
...
loadCSS = function(href) {
	...
	if($.inArray(href, HEAD_included_files) == -1) {
		var cssLink = $("<link rel='stylesheet' type='text/css' href='" + config.options.managerPath + href + "'>");
		$("head").append(cssLink);
...
loadJS = function(src) {
	...
	if($.inArray(src, HEAD_included_files) == -1) {
		var jsLink = $("<script type='text/javascript' src='" + config.options.managerPath + "/" + src + "'>");
	    $("head").append(jsLink);
...
var lg = [];
$.ajax({
  url: config.options.managerPath + '/scripts/languages/'  + config.options.culture + '.js',
  async: false,
  dataType: 'json',
...
var getFolderInfo = function(path) {
	...
	var loading = '<img id="activity" src="' + config.options.managerPath + '/themes/' + config.options.theme + '/images/wait30trans.gif" width="30" height="30" />';
...
$(function(){
	...
	$.ajax({
	    url: config.options.managerPath + '/themes/' + config.options.theme + '/styles/ie.css',
	    async: false,
	    success: function(data)
	    {
	        $('head').append(data);
